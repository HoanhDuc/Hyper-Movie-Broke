...
export default function Player({ option, src, getInstance, ...rest }: any) {
    ...
    if (src) {
      const art = new Artplayer({
        ...
        icons: {
          ...
          // state:
          //   '<img width="150" height="150" class="" src="/play-icon.png" />',
        },
        moreVideoAttr: {
        ...
        customType: {
          ...
          direct: function playDirect(video, url) {
            video.src = url;
          },
          embed: function playEmbed(video, url) {
            video.src = url;
          },
        },
      });
  ...
  return (
    <div
      ref={artRef}
      {...rest}
      className="w-full h-[250px] lg:h-[90vh] mb-3 cursor-pointer overflow-hidden shadow-xl"
    ></div>
  );
}
